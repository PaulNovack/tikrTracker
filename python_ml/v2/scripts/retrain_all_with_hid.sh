#!/bin/bash
# v2/scripts/retrain_all_with_hid.sh — Retrain all active pipelines with H+I+D combined.
# Run overnight. Identical to retrain_all.sh except H, I, D are trained as a single
# combined model (--pipeline H,I,D) instead of three separate models.
#
# Pipelines trained: A, B, C, D+H+I (combined), E, F, G, J, K, L, M, N, O, P, Q, R
# Note: P is forward-biased (v2100.0).
#
# Usage:
#   bash python_ml/v2/scripts/retrain_all_with_hid.sh
...
echo "=========================================================="
echo "  Retrain All Active Pipelines — H+I+D Combined"
...
# H+I+D — Combined High-Momentum, Intraday Reversal, Day Range Breakout
launch_job "Combined H+I+D" "HID-$(date +%Y-%m-%d_%H).log" \
    $TRAINER \
        --pipeline H,I,D --win-threshold 2.0 --actual-fill-weight 20.0 \
        --eval-on-actual-only --split-mode day \
        --start 2025-10-01 --end "$TODAY" \
        --test-size 0.2 --train-full \
...
PIPELINE_LETTERS="A B C E F G J K L M N O P Q R"
...
# ── H+I+D combined: extract AUC/P@10 from HID log and persist for H, I, D ──
HID_PATTERN="${LOG_DIR}/HID-$(date +%Y-%m-%d)_*.log"
HID_LATEST=$(ls -t $HID_PATTERN 2>/dev/null | head -1)
if [[ -n "$HID_LATEST" ]]; then
    HID_AUC=$(grep -oP 'Test AUC:\s*\K[\d.]+' "$HID_LATEST" | head -1)
    HID_P10_BT=$(grep -oP '\[bt_only\] Precision@10=\K[\d.]+' "$HID_LATEST" | head -1)
    HID_P10_ACTUAL=$(grep -oP 'Precision@10 \(actual\):\s*\K[\d.]+' "$HID_LATEST" | head -1)
    if [[ -n "$HID_P10_ACTUAL" && "$HID_P10_ACTUAL" != "0.000" ]]; then
        HID_P10="$HID_P10_ACTUAL"; HID_P10_SRC="actual"
    else
        HID_P10="$HID_P10_BT"; HID_P10_SRC="${HID_P10_BT:+BT}"
    fi
    for LOW in h i d; do
        if [[ -n "$HID_AUC" ]]; then
            if persist_setting "trading.pipeline_auc.${LOW}" "$HID_AUC"; then
                echo "   HID→${LOW}: AUC=${HID_AUC} ✔"
            else
                echo "   HID→${LOW}: AUC=${HID_AUC} ✗ (DB write failed)"
            fi
        fi
        if [[ -n "$HID_P10" ]]; then
            if persist_setting "trading.pipeline_p10.${LOW}" "$HID_P10"; then
                echo "     P@10=${HID_P10} (${HID_P10_SRC}) ✔"
            else
                echo "     P@10=${HID_P10} (${HID_P10_SRC}) ✗ (DB write failed)"
            fi
        fi
        # Persist dedicated ML updated timestamp for each pipeline fed by the HID model
