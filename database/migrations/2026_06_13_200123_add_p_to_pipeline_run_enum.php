...
return new class extends Migration
{
    ...
    {
        DB::statement("ALTER TABLE trade_alerts MODIFY COLUMN pipeline_run ENUM('A','B','C','D','E','F','G','H','I','J','K','L','M','N','O','P','X','BIASED1') NOT NULL DEFAULT 'A'");

        if (Schema::hasTable('trade_alerts_unfiltered')) {
            DB::statement("ALTER TABLE trade_alerts_unfiltered MODIFY COLUMN pipeline_run ENUM('A','B','C','D','E','F','G','H','I','J','K','L','M','N','O','P','X','BIASED1') NOT NULL DEFAULT 'A'");
        }

        if (Schema::hasTable('trade_alerts_ml_pick')) {
            DB::statement("ALTER TABLE trade_alerts_ml_pick MODIFY COLUMN pipeline_run ENUM('A','B','C','D','E','F','G','H','I','J','K','L','M','N','O','P','X','BIASED1') NOT NULL");
        }
    }
