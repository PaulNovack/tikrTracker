...
export default function NotableAssets({
    stagnationData,
    shortDays,
    longDays,
    flatThresholdPct,
    ...
    const downtrendAssets = stagnationData.filter(
        (asset) => asset.is_downtrend,
    );
    const stagnantAssets = stagnationData.filter(
        (asset) => asset.is_stagnant && !asset.is_downtrend && !asset.has_significant_gain,
    );
    const highPerformingAssets = stagnationData.filter(
        (asset) => asset.has_significant_gain && !asset.is_downtrend,
    );
    const activeAssets = stagnationData.filter(
        (asset) => !asset.is_stagnant && !asset.is_downtrend && !asset.has_significant_gain,
    );
            ...
            >
                <div className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-between">
                        <Heading
                            title="Notable Assets Analysis"
                            description="Analysis of top 500 most active assets organized into three key categories: High Performers (>2% daily gains), Stagnant Assets (≤1% daily movement), and Downtrend Assets (declining performance). Includes 5 lookback periods (30d, 15d, 5d, 3d, 1d) plus intraday 'Today' performance (9:30 AM EST to current time on market days after 10 AM EST). Momentum indicators compare 1D vs 3D performance."
                        />
                    </div>
                    ...
                    {highPerformingAssets.length > 0 && (
                        <StagnationTable
                            assets={highPerformingAssets}
                            title={`📈 High Performers (${highPerformingAssets.length})`}
                        />
                    )}

                    {/* Stagnant Assets */}
                    {stagnantAssets.length > 0 && (
                        <StagnationTable
                            assets={stagnantAssets}
                            title={`📊 Stagnant Assets (${stagnantAssets.length})`}
                        />
                    )}

                    {/* Downtrend Assets */}
                    {downtrendAssets.length > 0 && (
                        <StagnationTable
                            assets={downtrendAssets}
                            title={`🔴 Downtrend Assets (${downtrendAssets.length})`}
                            isDowntrend={true}
                        />
                    )}
                            ...
                            <CardContent className="flex flex-col items-center justify-center gap-4 py-12">
                                ...
                                <div className="text-center">
                                    ...
                                    <CardDescription className="mt-1">
                                        Check back when market data has been
                                        populated
                                    </CardDescription>
                                </div>
                            </CardContent>
