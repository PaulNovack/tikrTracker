...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Stale Entries', href: '/logs/stale-entries' },
];

export default function StaleEntriesLog() {
    ...
    const runSearch = useCallback(async (query: string) => {
        ...
        try {
            const response = await fetch(`/api/logs/stale-entries/search?q=${encodeURIComponent(query)}&context=6`);
    ...
    const fetchLog = async () => {
        try {
            const response = await fetch('/api/logs/stale-entries?lines=500');
    ...
    const handleDownload = () => {
        ...
        a.href = url;
        a.download = filename || 'stale-alerts.log';
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Stale Entries" />
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            <CardTitle>Stale Entries</CardTitle>
                            <CardDescription>
                        ...
                        <div className="relative">
                            <div className="relative mb-3">
                                ...
                                <Input
                                    ...
                                    placeholder="Search stale entries (2+ characters)..."
                            ...
                            {searchResult !== null ? (
                                ...
                                <pre className="bg-gray-900 text-green-400 p-4 rounded-lg overflow-auto h-[calc(100vh-320px)] text-xs font-mono">
                                    {logContent || 'No stale entries logged yet'}
                                    <div ref={logEndRef} />
                                </pre>
                            )}
                        </div>
