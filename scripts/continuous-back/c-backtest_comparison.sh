...
# Pipeline C Backtest - Continuous Loop (Single Thread)
# Tests the production algorithm configured in .env (TRADE_ALERT_C_VERSION)
...
while true; do
    ...
    LOG_FILE="$LARAVEL_ROOT/storage/logs/backtest-c-${TODAY}.log"
    exec >> "$LOG_FILE" 2>&1
    ...
    ALGO_VERSION=$(grep "^TRADE_ALERT_C_VERSION=" "$LARAVEL_ROOT/.env" | cut -d '=' -f2)

    echo "============================================================"
    echo "  PIPELINE C BACKTEST  [Iteration $ITERATION]"
    echo "============================================================"
    echo ""
    echo "Date:      $TODAY"
    ...
    php artisan trade:pipeline-c stock \
        --rolling-window \
