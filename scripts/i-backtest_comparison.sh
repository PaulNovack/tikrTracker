...
# Pipeline I Backtest
# Tests the production algorithm configured in .env (TRADE_ALERT_I_VERSION)
...
echo "════════════════════════════════════════════════════════════"
echo "  PIPELINE I BACKTEST"
echo "════════════════════════════════════════════════════════════"
echo ""
echo "📅 Trading Days: ${#DATES[@]}"
echo "⏰ Time Range: 09:40 - 15:30 EST (built-in to --backtest mode)"
echo "🔧 Algorithm version configured in .env: TRADE_ALERT_I_VERSION"
echo ""
echo "This runs the Pipeline I algorithm configured in your .env file."
echo "  1. Generate alerts using --backtest mode"
echo ""

# Parallel processing configuration
MAX_PARALLEL=${MAX_PARALLEL:-8}  # Run 4 dates in parallel (adjust based on CPU cores)
...
process_date() {
    ...
    TRADING_PIPELINE_I_USE_REDIS=false php artisan trade:pipeline-i stock \
        --backtest \
        --from="$date" \
        --to="$date" \
        --top=50 \
...
ALGO_VERSION=$(grep "^TRADE_ALERT_I_VERSION=" "$LARAVEL_ROOT/.env" | cut -d '=' -f2)
echo "Using algorithm version from .env: $ALGO_VERSION"
echo ""

echo "════════════════════════════════════════════════════════════"
echo "  ANALYZING BACKTEST RESULTS (Pipeline I)"
echo "════════════════════════════════════════════════════════════"
echo ""

php artisan analyze:trade-alerts-atr-immediate \
    --algo-version="$ALGO_VERSION" \
    --pipeline=I --write-results --show-details
