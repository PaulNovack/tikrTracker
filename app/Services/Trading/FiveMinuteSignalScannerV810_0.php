...
/**
 * Version 810.0 - EMA Momentum Pullback Scanner (LONG)
 *
 ...
 *
 * This identifies stocks in strong trends ready for continuation after shallow pullback.
 */
class FiveMinuteSignalScannerV810_0
{
    use HasPriceTables;
...
';
        ...
        if (empty($rows)) {
            \Log::info('[V810 Scanner] No rows returned from SQL query');

            return [];
        }

        \Log::info('[V810 Scanner] Processing '.count($rows).' raw rows');
        ...
        \Log::info('[V810 Scanner] Scored '.count($cands)." candidates, returning top {$topN}");
