...
/**
 * Version 55.1 - Quality-filtered Trend Compression Continuation scanner.
 *
 ...
 */
class FiveMinuteSignalScannerV55_1
{
    use HasPriceTables;

    private string $version = 'v55.1';

    private string $name = 'Quality Trend Compression Continuation';

    // All v55 scanner settings live in this class. Edit these properties to tune it.
    ...
    public float $maxAtrPct5m = 1.80;

    public float $minRecentRvol5m = 1.20;

    public float $minMove20mPct = 0.55;

    public float $maxMove20mPct = 3.50;

    public float $maxAboveVwapPct = 1.60;

    public float $minEmaSpreadPct = 0.05;

    public float $minEma9SlopePct = 0.030;

    public float $minCompressionRatio = 0.25;

    public float $maxCompressionRatio = 0.78;

    public float $maxFlagDepthPct = 1.20;

    public float $minCloseLocation = 0.60;

    public float $minGreenBarPct = 50.0;

    public float $maxDistanceFromEma9Atr = 1.00;

    public float $minScannerScore = 52.0;
    ...
    public function scan(
        ...
        $cacheKey = "scan_v55_1:{$table}:{$bucketTs}:{$lookbackMinutes}";
        ...
        foreach ($grouped as $symbol => $bars) {
            ...
            $out[] = [
                'symbol' => $symbol,
                'signal_type' => 'QUALITY_TREND_COMPRESSION_SETUP_5M_V55_1',
        ...
        if ($this->isDebugEnabled()) {
            Log::info('[ScannerV55_1] debug counters', array_merge($drops, [
    ...
    {
        $symbols = $this->buildIntradayUniverse('scan_v55_1:universe_symbols', null, $asOfTsEst, $skipCache);
