...
/**
 * Version 19.0 - Simplified 3 Consecutive Gains Filter
 * Base: V18.0
 * Purpose: Require 3 consecutive gaining (green) 1-minute bars
 *
 * v19.0 changes:
 * - SIMPLIFIED: Only check for 3 consecutive gaining minutes (close > open)
 * - REMOVED: Alligator indicator (too complex, causing 0 signals)
 * - REMOVED: 5-minute breakout confirmation
 * - Expected: Basic momentum confirmation with maximum signal generation
 *
 * Base v18.0 features removed for simplicity:
 * - Multiple entry patterns still available: VWAP_RECLAIM, BULL_FLAG, MA_SQUEEZE, EMA bounces
 */
class OneMinuteEntryFinderV20_0
{
    use HasPriceTables;
    ...
    public function findBestLong(
        ...
        for ($i = max(22, $idxSignal); $i < count($norm); $i++) {
            ...
            // v20.0: RSI filter added - require 50 < RSI < 70
            // Avoids overbought (RSI >= 70) and ensures bullish momentum (RSI > 50)
            if (! isset($rsi[$i]) || $rsi[$i] === null || $rsi[$i] <= 50.0 || $rsi[$i] >= 70.0) {
                continue;
            }
        ...
        // v19.0: 3-consecutive-gains already checked at start of findBestLong
        ...
        if ($best) {
            ...
            // Use 14-period ATR on 1-minute bars, 2.5x ATR trailing stop
            $atr = $this->calculateATR($norm, 14);
            $best['atr'] = round($atr, 6);
            $best['atr_pct'] = ((float) $best['entry'] > 0) ? ($atr / (float) $best['entry']) * 100.0 : 0.0;
            $best['suggested_trailing_stop'] = round($atr * 3.0, 6);
            $best['suggested_trailing_stop_pct'] = ((float) $best['entry'] > 0) ? ($atr * 3.0 / (float) $best['entry']) * 100.0 : 0.0;
        }
    ...
    /**
     * v18.0: Apply 5-minute breakout confirmation
     *
     * Filters entries to only those where:
     * 1. Current 1m price is ABOVE recent 5m high (true breakout)
     * 2. Last 5m candle has strong bullish body (>0.3%)
     *
     * This prevents chasing pullbacks and fake moves.
     */
    private function applyFiveMinuteBreakoutConfirmation(
        ...
        // Get 1-minute data for current price
        $oneMinSql = '
        ...
        $lastPrice = (float) $oneMinResult->last_price;

        // Get 5-minute data for recent high and last candle body
        $fiveMinSql = "
            SELECT 
        ...
        $recent5High = (float) $fiveMinResult->recent5_high;
        $last5Open = (float) $fiveMinResult->last5_open;
        $last5Close = (float) $fiveMinResult->last5_close;

        // Check breakout confirmation
        $above5mHigh = $lastPrice > $recent5High;

        // Check strong bullish body on last 5m candle
        $bodyPct = $last5Open > 0 ? (($last5Close - $last5Open) / $last5Open) * 100.0 : 0;
        $strongBody5m = $bodyPct >= 0.3;

        if (! $above5mHigh || ! $strongBody5m) {
            return []; // Failed breakout confirmation
        }

        // All candidates pass the same breakout filter (it's symbol-level, not entry-level)
        // Add metadata to show confirmation
        foreach ($candidates as &$candidate) {
            $candidate['breakout_confirmed_5m'] = true;
            $candidate['above_5m_high'] = $above5mHigh;
            $candidate['strong_body_5m'] = $strongBody5m;
            $candidate['last_price_1m'] = $lastPrice;
            $candidate['recent5_high'] = $recent5High;

            // Boost score by 20% for confirmed breakouts
            if (isset($candidate['score'])) {
                $candidate['score'] = round($candidate['score'] * 1.2, 3);
            }
        }
    ...
    /**
     * v19.0 SIMPLIFIED: Check for 3 consecutive gaining minutes with quality
     *
     * Requirements:
     * 1. Last 3 bars must be green (close > open)
     * 2. Total 3-minute gain must be at least 0.3%
     * 3. Last bar must have volume spike (1.5x average of prior 10)
     *
    ...
    private function hasThreeConsecutiveGains(
        ...
        // Check last 3 bars are all gaining (close > open)
        $consecutiveGains = 0;
        for ($i = 10; $i < 13; $i++) {
            if ($bars[$i]->price > $bars[$i]->open) {
                $consecutiveGains++;
            }
        }

        if ($consecutiveGains < 3) {
            return false; // Not 3 consecutive gains
        }

        // Calculate total 3-minute gain
        $startPrice = $bars[9]->price; // Price 3 bars ago
        $endPrice = $bars[12]->price; // Latest price
        $gainPct = (($endPrice - $startPrice) / $startPrice) * 100;

        if ($gainPct < 0.3) {
            return false; // Total gain too small (< 0.3%)
        }

        // Check volume surge on last bar
        $lastVol = $bars[12]->volume;
        $avgVol = 0.0;
        for ($i = 2; $i < 12; $i++) {
        ...
        $volRatio = $avgVol > 0 ? ($lastVol / $avgVol) : 1.0;

        if ($volRatio < 1.5) {
            return false; // No volume surge (< 1.5x average)
        }
    ...
    /**
     * v19.0: Apply Alligator WAKE_UP confirmation
     *
     ...
     *
     * WAKE_UP state: lips > teeth > jaw, price between lips & teeth
     * This indicates early bullish trend formation
     *
     * Requirement: 3 consecutive WAKE_UP bars before entry accepted
     */
    private function applyAlligatorWakeUpConfirmation(
        ...
        // Get last 200 bars for Alligator calculation (need warmup for SMMA)
        $bars = $this->dbSelect('
            SELECT price, ts_est
            FROM one_minute_prices
            WHERE asset_type = ? AND symbol = ?
        ...
        $alligator = $this->calculateAlligator($bars);

        // Check for 3 consecutive WAKE_UP states at end
        $recentStates = array_slice($alligator, -3);

        if (count($recentStates) < 3) {
            return []; // Not enough calculated states
        }
        ...
        if (! $allWakeUp) {
            return []; // Did not have 3 consecutive WAKE_UP states
        }

        // All candidates pass - add metadata
        $lastState = end($recentStates);
        foreach ($candidates as &$candidate) {
            $candidate['alligator_wake_up'] = true;
            $candidate['alligator_consecutive'] = 3;
            $candidate['alligator_lips'] = $lastState['lips'];
            $candidate['alligator_teeth'] = $lastState['teeth'];
            $candidate['alligator_jaw'] = $lastState['jaw'];

            // Boost score by 30% for Alligator WAKE_UP
            if (isset($candidate['score'])) {
                $candidate['score'] = round($candidate['score'] * 1.3, 3);
            }
        }
    ...
    /**
     * Calculate Bill Williams Alligator indicator
     *
     * Returns array of states with Alligator lines and current state
     */
    private function calculateAlligator(array $bars): array
    {
        $prices = array_map(fn ($b) => (float) $b->price, $bars);

        // Calculate SMMA lines
        $lips = $this->calculateSMMA($prices, 5); // Shift +3 for plotting
        $teeth = $this->calculateSMMA($prices, 8); // Shift +5 for plotting
        $jaw = $this->calculateSMMA($prices, 13); // Shift +8 for plotting

        $result = [];
        foreach ($bars as $i => $bar) {
            // Apply shifts for alignment (forward shift for plotting)
            $lipsIdx = $i - 3; // Lips shifted +3
            $teethIdx = $i - 5; // Teeth shifted +5
            $jawIdx = $i - 8; // Jaw shifted +8

            $lipsVal = ($lipsIdx >= 0 && isset($lips[$lipsIdx])) ? $lips[$lipsIdx] : null;
            $teethVal = ($teethIdx >= 0 && isset($teeth[$teethIdx])) ? $teeth[$teethIdx] : null;
            $jawVal = ($jawIdx >= 0 && isset($jaw[$jawIdx])) ? $jaw[$jawIdx] : null;

            if ($lipsVal === null || $teethVal === null || $jawVal === null) {
                continue; // Skip until all lines available
            }
            ...
            // WAKE_UP (Bull): lips > teeth > jaw, price between lips & teeth
            if ($lipsVal > $teethVal && $teethVal > $jawVal && $price >= $teethVal && $price <= $lipsVal) {
                $state = 'WAKE_UP';
            }
            // EATING (Bull): price > lips > teeth > jaw
            elseif ($price > $lipsVal && $lipsVal > $teethVal && $teethVal > $jawVal) {
                $state = 'EATING';
            }
            // SLEEPING: All lines converged
            elseif (abs($lipsVal - $teethVal) / $price < 0.0015 && abs($teethVal - $jawVal) / $price < 0.0015) {
                $state = 'SLEEPING';
            }
            ...
            $result[] = [
                'ts_est' => $bar->ts_est,
                'price' => $price,
                'lips' => round($lipsVal, 6),
                'teeth' => round($teethVal, 6),
                'jaw' => round($jawVal, 6),
                'state' => $state,
            ];
        }
     ...
     * - Subsequent: (prev_smma * (N-1) + current_price) / N
     */
    private function calculateSMMA(array $prices, int $period): array
    {
        $result = [];
