...
/**
 * Version 55.3 - Practical Trend Compression Continuation scanner.
 *
 ...
 */
class FiveMinuteSignalScannerV55_3
{
    use HasPriceTables;

    private string $version = 'v55.3';

    private string $name = 'Practical Trend Compression Continuation';

    // All v55.3 scanner settings live in this class. Edit these properties to tune it.
    ...
    public float $minAtrPct5m = 0.18;

    public float $maxAtrPct5m = 2.50;

    public float $minRecentRvol5m = 0.90;

    public float $minMove20mPct = 0.25;

    public float $maxMove20mPct = 5.00;

    public float $maxAboveVwapPct = 2.25;

    public float $minEmaSpreadPct = 0.01;

    public float $minEma9SlopePct = 0.005;

    public float $minCompressionRatio = 0.10;

    public float $maxCompressionRatio = 1.10;

    public float $maxFlagDepthPct = 2.00;

    public float $minCloseLocation = 0.42;

    public float $minGreenBarPct = 33.0;

    public float $maxDistanceFromEma9Atr = 1.75;

    public float $minScannerScore = 36.0;

    public int $activeWindowMinutes = 15;
    ...
    public function scan(
        ...
        $cacheKey = "scan_v55_3:{$table}:{$bucketTs}:{$lookbackMinutes}";
        ...
        foreach ($grouped as $symbol => $bars) {
            ...
            if (
                ...
                || (float) $metrics['last_close'] < ((float) $metrics['ema9'] - (0.30 * (float) $metrics['atr']))
            ) {
                $drops['trend']++;
            ...
            $liquidityOk =
                ...
                && (float) $metrics['notional_last5m'] >= ($minNotional5m * 0.45);
            if (! $liquidityOk) {
                $drops['liquidity']++;
            ...
            $out[] = [
                'symbol' => $symbol,
                'signal_type' => 'PRACTICAL_TREND_COMPRESSION_SETUP_5M_V55_3',
        ...
        if ($this->isDebugEnabled()) {
            Log::info('[ScannerV55_3] debug counters', array_merge($drops, [
    ...
    {
        $symbols = $this->buildIntradayUniverse('scan_v55_3:universe_symbols', null, $asOfTsEst, $skipCache);
