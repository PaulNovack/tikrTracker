...
/**
 * Version 55.1 - Quality-filtered Trend Compression Continuation entry finder.
 *
 ...
 */
class OneMinuteEntryFinderV55_1
{
    use HasPriceTables;

    private string $version = 'v55.1';

    // All v55 entry settings live in this class. Edit these properties to tune it.
    ...
    public int $minBars = 24;

    public int $atrPeriod1m = 14;

    public int $volLookback1m = 20;

    public int $maxEntryAgeMinutes = 1;

    public int $maxSignalAgeMinutes = 6;

    public float $minNotional1m = 100000.0;

    public float $minVolumeRatio = 1.35;

    public float $minBreakoutVsFlagVolumeRatio = 1.40;

    public float $minBodyPct = 0.10;

    public float $minBodyRangeFraction = 0.55;

    public float $minClosePosition = 0.78;

    public float $maxUpperWickFraction = 0.20;

    public float $maxAboveVwapPct = 1.30;

    public float $minAtrPct = 0.18;

    public float $maxAtrPct = 1.60;

    public float $minEmaSpreadPct = 0.04;

    public float $minEma9SlopePct = 0.015;

    public int $minFlagBars = 4;

    public int $maxFlagBars = 7;

    public float $maxFlagRangePct = 0.90;

    public float $maxFlagRangeAtr = 1.05;

    public float $breakBufferPct = 0.03;

    public float $maxBreakDistanceAtr = 0.30;

    public float $maxPullbackBelowVwapPct = 0.10;


    public int $impulseLookbackBars = 3;

    public float $minImpulsePct = 0.35;

    public float $minImpulseAtr = 1.00;

    public float $maxFlagRetraceFraction = 0.55;

    public float $maxFlagCompressionRatio = 0.78;

    public float $maxFlagToImpulseVolumeRatio = 0.90;

    public bool $allowEma9PivotBreak = false;
    ...
    public float $maxPivotDistanceFromEma9Atr = 0.35;

    public float $stopAtrBuffer = 0.20;

    public float $stopPriceBufferPct = 0.05;

    public float $minStopPct = 0.50;

    public float $maxStopPct = 1.50;

    public float $minRoomToSessionHighR = 1.25;
    ...
    public float $minEntryScore = 58.0;
    ...
    public int $morningEndMinute = 640;        // 10:40

    public int $afternoonStartMinute = 900;    // 15:00
    ...
    public float $lateMorningScoreMultiplier = 0.90;

    public float $earlyAfternoonScoreMultiplier = 0.88;

    public float $powerHourScoreMultiplier = 0.95;
    ...
    public function findBestLong(
        ...
        return [
            ...
            'meta' => [
                'version' => $this->version,
                'pattern' => 'QUALITY_TREND_COMPRESSION_CONTINUATION',
                'as_of_ts_est' => $asOfTsEst,
            ],
        ];
    }
    ...
    private function findEntry(
        ...
        for ($i = $lastIndex; $i >= $earliestIndex; $i--) {
            ...
            if ((float) $scoreParts['score'] < $minEntryScore) {
        ...
        if ($candidates === []) {
            return [null, 'no_valid_v55_1_entry'];
        }
        ...
        if ($this->isDebugEnabled()) {
            Log::info('[EntryFinderV55_1] accepted entry', [
                'symbol' => $symbol,
    ...
    private function maybeLogDebug(): void
    {
        ...
        if (self::$dbg['called'] > 0 && self::$dbg['called'] % $every === 0) {
            Log::info('[EntryFinderV55_1] debug counters', array_merge(self::$dbg, [
                'pid' => getmypid(),
                'debug_every' => $every,
            ]));
        }
