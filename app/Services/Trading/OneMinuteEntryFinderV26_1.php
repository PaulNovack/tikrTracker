...
/**
 * Version 26.0 - Entry Finder
 * Base: V17.0
 * Purpose: Find best entry before forced entry time, or force entry at configured time
 * Changes:
 * - Only scan for entries before forced entry time (configurable, default 1:00 PM)
 * - If no quality entry found, create forced entry at configured time at market price
 */
class OneMinuteEntryFinderV26_1
{
    use HasPriceTables;
    ...
    public function findBestLong(
        ...
        // v26.0: Get forced entry time from config (default 13:00 = 1 PM)
        $forceTime = config('trading.v26.entry.force_entry_time', '13:00');
        $forceEntryTime = $tradeDate.' '.$forceTime.':00';

        // v26.0: Only consider entries before forced entry time for quality picks
        // After forced time, we'll create a forced entry if no quality entry was found
        $filtered = array_filter($filtered, function ($c) use ($forceEntryTime) {
            return $c['entry_ts_est'] < $forceEntryTime;
        });

        usort($filtered, fn ($a, $b) => ($b['score'] <=> $a['score']));
        $best = $filtered[0] ?? null;

        // v26.0: If no entry found before forced time, force entry at configured time
        if (! $best && $asOfTsEst >= $forceEntryTime) {
            // Find the bar at or closest to forced entry time
            $forceBar = null;
            $forceIdx = null;
            foreach ($norm as $idx => $bar) {
                if ($bar['ts_est'] >= $forceEntryTime) {
                    $forceBar = $bar;
                    $forceIdx = $idx;
                    break;
                }
            ...
            if ($forceBar) {
                // v26.0: Calculate metrics for forced entry (no rejection filters)
                // If scanner approved this symbol (4+ positive days), trust it and enter

                // Calculate ATR for stop distance
                $atr = $this->calculateATR($norm, 14);
                $atrPct = ($forceBar['close'] > 0) ? ($atr / $forceBar['close']) * 100.0 : 0.0;

                // Calculate momentum from market open
                $openPrice = $norm[0]['open']; // First bar (market open)
                $momentumPct = ($openPrice > 0) ? (($forceBar['close'] - $openPrice) / $openPrice) * 100.0 : 0.0;

                // Calculate volume ratio
                $baseVol = 0.0;
                $volCount = 0;
                for ($j = max(0, $forceIdx - 20); $j < $forceIdx; $j++) {
                    $baseVol += $norm[$j]['volume'];
                    $volCount++;
                }
                $avgVol = $volCount > 0 ? $baseVol / $volCount : $forceBar['volume'];
                $volRatio = $avgVol > 0 ? $forceBar['volume'] / $avgVol : 1.0;

                // Create forced entry (no rejections - scanner already filtered)
                $entryPrice = $forceBar['close'];

                // v26.0: Use ATR for stop distance, not bar low
                // Calculate stop as entry - (ATR%), with minimum 0.7% and maximum 1.0%
                $stopDistancePct = max(0.7, min($atrPct, 1.0));
                $stop = $entryPrice * (1 - ($stopDistancePct / 100));

                $best = [
                    'type' => 'FORCED1PM_ENTRY',
                    'trigger_ts_est' => $forceEntryTime,
                    'entry_ts_est' => $forceEntryTime,
                    'entry' => round($entryPrice, 6),
                    'stop' => round($stop, 6),
                    'vol_ratio' => round($volRatio, 3),
                    'score' => 5.0, // Default score for forced entries
                    'notes' => 'Forced 11AM entry: ATR='.round($atrPct, 2).'%, Momentum=+'.round($momentumPct, 2).'%, Vol='.round($volRatio, 1).'x, Stop='.round($stopDistancePct, 2).'%',
                ];
            }
        }

        // v27.0: DATA-DRIVEN FILTER - Based on trade_alerts JOIN trade_alerts_unfiltered
        // Top types: BOTTOM_ENTRY (5.87% P/L), BULL_FLAG_BREAK (1.44% P/L, 70.1% WR)
        // Score 40-59: 5.40% avg P/L! Score 80-100: 1.28% P/L, 68.9% WR
        // REJECT: Score 20-39 (-0.16% P/L), Score 60-79 (0.44% P/L)
        $candidates = array_filter($candidates, function ($c) {
            ...
            // ONLY accept proven winners with positive avg P/L and 56%+ win rate
            $allowedTypes = [
                'BOTTOM_ENTRY',      // 5.87% P/L - BEST!
                'EMA21_BOUNCE',      // 1.55% P/L, 100% WR
                'MA_SQUEEZE',        // 1.23% P/L, 100% WR
                'OR_BREAKOUT',       // 1.18% P/L, 83.3% WR
                'ORB_RETEST',        // 1.05% P/L, 100% WR
                'BULL_FLAG_BREAK',   // 1.44% P/L, 70.1% WR
                'VWAP_RECLAIM_1M',   // 1.61% P/L, 64.3% WR
                'EMA_CROSS_BULL',    // 1.30% P/L, 74.1% WR
                'BREAKOUT_RETEST',   // 1.62% P/L, 56.5% WR
            ];
            if (! in_array($type, $allowedTypes)) {
                return false;
            }

            // Score filter: 40-59 (BEST: 5.40% P/L) OR 80-100 (Good: 1.28% P/L)
            // REJECT: 0-39 and 60-79
            if ($score >= 40.0 && $score < 60.0) {
                // Golden range
            } elseif ($score >= 80.0 && $score <= 100.0) {
                // Good range
            } else {
                return false;
            }

            // Avoid 3pm hour (48.4% win rate)
            if ($hour >= 15) {
                return false;
            }

            // Type-specific filters based on loser analysis
            if ($type === 'BOTTOM_ENTRY' && $score < 40.0) {
                return false;
            }
            if ($type === 'BULL_FLAG_BREAK' && $score < 80.0) {
                return false; // 22 losers with avg score 83.5
            }
            if ($type === 'VWAP_RECLAIM_1M' && $score < 50.0) {
                return false; // 10 losers with avg score 56.8
            }

            return true;
        });

        // Re-sort candidates after filtering and pick best
        ...
        if ($best) {
            ...
            // v12.4: Calculate ATR (Average True Range) for volatility measurement
            // Use fixed 3% trailing stop for forced 11AM entries (room for morning chop)
            $atr = $this->calculateATR($norm, 14);
            $best['atr'] = round($atr, 6);
            $best['atr_pct'] = ((float) $best['entry'] > 0) ? ($atr / (float) $best['entry']) * 100.0 : 0.0;
            $best['suggested_trailing_stop'] = round((float) $best['entry'] * 0.03, 6);
            $best['suggested_trailing_stop_pct'] = 3.0;
        }
