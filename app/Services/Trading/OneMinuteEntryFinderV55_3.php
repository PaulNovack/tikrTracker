...
/**
 * Version 55.3 - Practical Trend Compression Continuation entry finder.
 *
 ...
 */
class OneMinuteEntryFinderV55_3
{
    use HasPriceTables;

    private string $version = 'v55.3';

    // All v55.3 entry settings live in this class. Edit these properties to tune it.
    ...
    public int $minBars = 14;

    public int $atrPeriod1m = 14;

    public int $volLookback1m = 20;

    public int $maxEntryAgeMinutes = 4;

    public int $maxSignalAgeMinutes = 15;

    public float $minNotional1m = 40000.0;

    public float $minVolumeRatio = 1.00;

    public float $minBreakoutVsFlagVolumeRatio = 0.95;

    public float $minBodyPct = 0.04;

    public float $minBodyRangeFraction = 0.35;

    public float $minClosePosition = 0.60;

    public float $maxUpperWickFraction = 0.40;

    public float $maxAboveVwapPct = 2.00;

    public float $minAtrPct = 0.10;

    public float $maxAtrPct = 2.50;

    public float $minEmaSpreadPct = 0.005;

    public float $minEma9SlopePct = 0.0000;

    public int $minFlagBars = 3;

    public int $maxFlagBars = 7;

    public float $maxFlagRangePct = 1.60;

    public float $maxFlagRangeAtr = 2.00;

    public float $breakBufferPct = 0.01;

    public float $maxBreakDistanceAtr = 0.75;

    public float $maxPullbackBelowVwapPct = 0.40;

    public int $impulseLookbackBars = 3;

    public float $minImpulsePct = 0.15;

    public float $minImpulseAtr = 0.40;

    public float $maxFlagRetraceFraction = 0.85;

    public float $maxFlagCompressionRatio = 1.20;

    public float $maxFlagToImpulseVolumeRatio = 1.35;

    public bool $allowEma9PivotBreak = true;

    public int $pivotLookbackBars = 3;

    public int $pivotOlderLookbackBars = 4;

    public float $maxPivotDepthPct = 2.20;

    public float $maxPivotDistanceFromEma9Atr = 0.85;
    ...
    public float $maxStopPct = 1.80;

    public float $minRoomToSessionHighR = 0.50;
    ...
    public float $minEntryScore = 42.0;

    public float $minPivotEntryScore = 44.0;
    ...
    public int $afternoonEndMinute = 940;      // 15:40
    ...
    private function findEntry(
        ...
        if ($candidates === []) {
            return [null, 'no_valid_v55_3_entry'];
        }
        ...
        if ($this->isDebugEnabled()) {
            Log::info('[EntryFinderV55_3] accepted entry', [
                'symbol' => $symbol,
    ...
    private function detectEma9PivotBreak(
        ...
        $recentLowIndex = $this->indexOfMinField($bars, $recentStart, $recentEnd, 'low');

        if ($pivotHigh <= 0 || $recentLow <= 0 || $olderLow <= 0 || $atr <= 0 || $recentLowIndex === null) {
            return null;
        }

        $pullbackAtr = max(1e-9, (float) $bars[$recentLowIndex]['atr']);
        $pullbackEma9 = (float) $bars[$recentLowIndex]['ema9'];
        $previousClose = (float) $bars[$index - 1]['close'];
        ...
        $distanceFromEma9Atr = abs($recentLow - $pullbackEma9) / $pullbackAtr;
    ...
    private function indexOfMinField(array $bars, int $start, int $end, string $field): ?int
    {
        ...
        for ($i = $start + 1; $i <= $end; $i++) {
            if (isset($bars[$i]) && (float) $bars[$i][$field] < $bestValue) {
                $bestValue = (float) $bars[$i][$field];
                $bestIndex = $i;
            }
        }
    ...
    private function maybeLogDebug(): void
    {
        ...
        if (self::$dbg['called'] > 0 && self::$dbg['called'] % $every === 0) {
            Log::info('[EntryFinderV55_3] debug counters', array_merge(self::$dbg, [
                'pid' => getmypid(),
                'debug_every' => $every,
            ]));
        }
