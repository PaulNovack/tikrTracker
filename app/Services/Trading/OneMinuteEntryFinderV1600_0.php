...
/**
 * Version 1600.0 - Entry Finder (Early Momentum)
 *
 * Entry types (long):
 ...
 */
class OneMinuteEntryFinderV1600_0 extends AbstractOneMinuteEntryFinder
{
    ...
    {
        return ((string) env('ENTRYFINDER_V1600_DEBUG', '0') === '1')
    ...
    protected function maybeLogDebug(): void
    {
        ...
        if (self::$dbg['called'] > 0 && self::$dbg['called'] % 20000 === 0) {
            Log::info('[EntryFinderV1600] debug counters', self::$dbg);
        }
    }
    ...
    private function findEntry(string $symbol, string $assetType, string $signalTsEst, string $asOfTsEst): ?array
    {
        ...
        $minNotional1m = (float) ($cfg['min_notional_1m'] ?? 35000.0);
        $maxAboveVwapPct = (float) ($cfg['max_above_vwap_entry_pct'] ?? 1.6);
        $minRoomPct = (float) ($cfg['min_room_to_run_pct'] ?? 0.35);
        $roomAtrMult = (float) ($cfg['room_atr_mult'] ?? 1.0);
        $minVolRatio = (float) ($cfg['min_vol_ratio_1m'] ?? 0.85);
        $volLookback = (int) ($cfg['vol_lookback_1m'] ?? 20);
        $minBodyPct = (float) ($cfg['min_body_pct_1m'] ?? 0.03);
        $maxEntryAgeMinutes = (int) ($cfg['max_entry_age_minutes'] ?? 10);

        $emaFast = (int) ($cfg['ema_fast'] ?? 9);
        $emaSlow = (int) ($cfg['ema_slow'] ?? 21);
        $atrPeriod = (int) ($cfg['atr_period_1m'] ?? 14);

        $beforeMinutes = (int) ($cfg['analysis_lookback_minutes'] ?? 120);
        ...
        usort($candidates, function ($a, $b) {
            // Primary: most recent entry time first
        ...
        $maxSignalAgeMinutes = (int) config('trading.v1600.scanner.active_window_minutes', 6);
