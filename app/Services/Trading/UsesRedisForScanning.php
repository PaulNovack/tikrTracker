...
trait UsesRedisForScanning
{
    ...
    protected function doScan(
        ...
        $asOfEpochRaw = strtotime($asOfTsEst.' America/New_York');
        $asOfEpoch = $asOfEpochRaw !== false
            ? strtotime(date('Y-m-d H:i:00', $asOfEpochRaw).' America/New_York')
            : false;
        if ($asOfEpoch === false) {
            return [];
        }
        ...
        foreach ($allBars as $sym => $bars) {
            ...
            // Age check
            $signalAgeSeconds = $asOfEpoch - (int) strtotime($lastTsEst.' America/New_York');
        ...
        $result = array_slice($out, 0, max(1, $limit));

        \Log::channel('redis-scan')->info('[Scanner '.$this->getVersion().'] Redis batch scan complete', [
            'as_of' => $asOfTsEst,
            'universe_size' => count($symbols),
            'total_bars' => array_sum(array_map(static fn ($b) => count($b), $allBars)),
            'signals_found' => count($result),
            'top_score' => $result[0]['score'] ?? null,
            'top_symbol' => $result[0]['symbol'] ?? null,
        ]);
    ...
    protected function getYesterdayClose(string $symbol, string $tradeDate): ?float
    {
        ...
        $cached = \Illuminate\Support\Facades\Redis::connection('rt')->get($cacheKey);
        if ($cached !== null) {
            return (float) $cached;
        }
        ...
        if ($close !== null) {
            \Illuminate\Support\Facades\Redis::connection('rt')->setex($cacheKey, 86400, (string) $close);
        }
    ...
    public function scanSymbol(string $symbol, string $asOfTsEst): ?array
    {
        ...
        $asOfDt = new \DateTime($asOfTsEst, $ny);
        $fromDt = (clone $asOfDt)->modify('-'.($barsNeeded * 300).' seconds');
        $bars = $this->redisRepo()->getBars('5m', $symbol, 'stock', $fromDt->format('Y-m-d H:i:s'), $asOfTsEst, $barsNeeded);

        if (count($bars) < $moveBars + 2) {
            $redisKey = sprintf('rt:bars:5m:%s:stock:%s', str_replace('-', '', $fromDt->format('Y-m-d')), strtoupper($symbol));
            $fromEpoch = strtotime($fromDt->format('Y-m-d H:i:s').' America/New_York');
            $toEpoch = strtotime($asOfTsEst.' America/New_York');

            \Log::channel('redis-scan')->info('[Scanner '.$this->getVersion().'] No bars', [
                'key' => $redisKey,
                'symbol' => $symbol,
                'bars' => count($bars),
                'needed' => $moveBars + 2,
                'fromEpoch' => $fromEpoch,
                'toEpoch' => $toEpoch,
                'fromEst' => $fromDt->format('Y-m-d H:i:s'),
                'toEst' => $asOfTsEst,
            ]);
        ...
        $lastTsEpoch = (int) strtotime($lastTsEst.' America/New_York');
        $signalAgeSeconds = $asOfEpoch - $lastTsEpoch;
        $maxSignalAgeSeconds = max(1, $activeWindowMinutes) * 60;
        if ($signalAgeSeconds < 0 || $signalAgeSeconds > $maxSignalAgeSeconds || $lastClose <= 0) {
            \Log::channel('redis-scan')->info('[Scanner '.$this->getVersion().'] Signal stale', [
                'symbol' => $symbol,
                'age' => $signalAgeSeconds,
                'max' => $maxSignalAgeSeconds,
                'event_ts' => $asOfTsEst,
                'last_bar_ts' => $lastTsEst,
                'bars_found' => $barCount,
            ]);
        ...
        // Gates
        if ($notional < $minNotional5m) {
            \Log::channel('redis-scan')->info('[Scanner '.$this->getVersion().'] Gate REJECT', [
                'symbol' => $symbol,
                'reason' => 'notional',
                'symbol' => $symbol,
                'reason' => 'notional',
        ...
        if (! ($rvolRatio >= $minRvol5m || $move30m >= $minMove30m)) {
            \Log::channel('redis-scan')->info('[Scanner '.$this->getVersion().'] Gate REJECT', [
                'symbol' => $symbol,
                'reason' => 'activity',
                'rvol' => round($rvolRatio, 2),
                'move30m' => round($move30m, 2),
                'min_rvol' => $minRvol5m,
                'min_move' => $minMove30m,
            ]);
