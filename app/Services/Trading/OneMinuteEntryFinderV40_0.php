...
/**
 * Version 40.0 - Runner Entry Finder
 * Purpose: Find best entries on stocks making sustained runs
 * Strategy: Enter on pullbacks or micro-consolidations during uptrends
 * Patterns:
 ...
 */
class OneMinuteEntryFinderV40_0
{
    use HasPriceTables;
    ...
    public function findBestLong(
        ...
        Log::channel('trading')->info("[V40.0] {$symbol}: Found ".count($candidates).' candidates, '.count($filtered).' after basic filters', [
            'symbol' => $symbol,
        ...
        $filtered = array_filter($filtered, function ($c) use ($symbol) {
            ...
            if (isset($c['entry']) && isset($c['stop'])) {
                ...
                if ($risk > 3.0) {
                    Log::channel('trading')->debug("[V40.0] {$symbol} REJECTED: Risk too high", [
                        'symbol' => $symbol,
                        'type' => $c['type'],
            ...
            if ($c['score'] < $minScore) {
                Log::channel('trading')->debug("[V40.0] {$symbol} REJECTED: Score too low", [
                    'symbol' => $symbol,
                    'type' => $c['type'],
                    'score' => $c['score'],
            ...
            if ($entryTime) {
                ...
                if ($hour === 9 && $minute < 45) {
                    Log::channel('trading')->debug("[V40.0] {$symbol} REJECTED: Too early", [
                        'symbol' => $symbol,
        ...
        Log::channel('trading')->info("[V40.0] {$symbol}: After quality filters", [
            'symbol' => $symbol,
        ...
        // Log selected entry
        Log::channel('trading')->info("[V40.0] {$symbol}: Selected entry", [
            'symbol' => $symbol,
            'type' => $best['type'],
