...
/**
 * Version 14.0 - Signal Scanner (Copy of V13.0)
 * Identical to V13.0 for baseline comparison and future modifications:
 * - Expanded universe: 500 stocks (5-day lookback) + 50 losers (previous day)
 * - Relaxed relative strength filter: 1.2x SPY
 * - Universe: Top performers + bounce plays
 */
class FiveMinuteSignalScannerV14_0
{
    use HasPriceTables;

    private string $version = 'v14.0';

    private string $name = 'Intraday Swing';
    ...
    public function scan(string $assetType, string $asOfTsEst, int $lookbackMinutes = 60, float $minMovePct = 0.6, float $volMult = 3.0, int $limit = 60): array
    {
        ...
        // Extract symbols from top performers
        $symbols = array_column($topPerformers, 'symbol');

        // Step 1b: V14.0 - Top 50 losers from previous trading day with $1.50-$100 price filter (bounce opportunities)
        try {
            ...
            if ($prevTradingDay) {
                // Get top 50 losers from previous day with price filter
                ...
                // Merge with existing symbols (avoiding duplicates)
                $symbols = array_unique(array_merge($symbols, $loserSymbols));
            }
        } catch (\Exception $e) {
            // If we can't get losers, continue with just top performers
            // This ensures the scanner doesn't fail if there's an issue
        }
...
";
        ...
        foreach ($rows as $r) {
            ...
            // v13.0: Relaxed RS filter to 1.2x (was 1.5x) to expand universe
            // If SPY is flat or down, allow any positive stock move
            // If SPY is up, require stock move > SPY move * 1.2
            if ($spyMovePct > 0 && $stockMovePct < $spyMovePct * 1.2) {
