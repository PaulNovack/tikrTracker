...
class FiveMinuteSignalScannerV1500_0Redis extends FiveMinuteSignalScannerV1500_0
{
    ...
    public function scanSymbol(string $symbol, string $asOfTsEst): ?array
    {
        ...
        $fromTsEst = $tradeDate.' 09:30:00';

        // Opening Range Breakout requires market hours; skip premarket bars
        $asOfEpoch = strtotime($asOfTsEst.' America/New_York');
        $openEpoch = strtotime($fromTsEst.' America/New_York');
        if ($asOfEpoch < $openEpoch) {
            return null;
        }

        $bars = $this->redisRepo()->getBars('5m', $symbol, 'stock', $fromTsEst, $asOfTsEst, 80);
