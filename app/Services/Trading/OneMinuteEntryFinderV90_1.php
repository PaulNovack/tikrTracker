 ...
 */
class OneMinuteEntryFinderV90_1 extends AbstractOneMinuteEntryFinder
{
    ...
    protected function doFindBestLong(
        ...
        $minScore = (float) config('trading.v90.entry_score_min', 94);
        ...
        // V91.0: DATA-DRIVEN FILTER - Based on trade_alerts JOIN trade_alerts_unfiltered analysis
        // Top Performers: BOTTOM_ENTRY (5.87% P/L!), OR_BREAKOUT (1.18% P/L, 83.3% WR),
        // VWAP_RECLAIM_1M (1.61% P/L, 64.3% WR), BULL_FLAG_BREAK (1.44% P/L, 70.1% WR)
        // Hidden Gems: EMA21_BOUNCE (1.55% P/L, 100% WR), MA_SQUEEZE (1.23% P/L, 100% WR)
        // CRITICAL: Score 40-59 = 5.40% avg P/L! Score 80-100 = 1.28% P/L
        ...
        $candidates = array_filter($candidates, function ($c) {
            ...
            // ONLY accept proven winners (100% acceptance rate in historical data means all were good)
            $allowedTypes = [
                'BOTTOM_ENTRY',      // 5.87% P/L - BEST!
                'OR_BREAKOUT',       // 1.18% P/L, 83.3% WR
                'VWAP_RECLAIM_1M',   // 1.61% P/L, 64.3% WR
                'BULL_FLAG_BREAK',   // 1.44% P/L, 70.1% WR
                'EMA21_BOUNCE',      // 1.55% P/L, 100% WR (hidden gem)
                'MA_SQUEEZE',        // 1.23% P/L, 100% WR (hidden gem)
                'ORB_RETEST',        // 1.05% P/L, 100% WR (hidden gem)
            ...
            // DATA-DRIVEN SCORE FILTER (relaxed for volume):
            // Accept 40+ to increase picks while maintaining quality
            // Still reject very low scores <40
            if ($score < 40.0) {
                return false;
            }

            // Avoid 3pm hour (48.4% win rate from previous analysis)
            if ($hour >= 15) {
                return false;
            }

            // Type-specific fine-tuning based on losing trade patterns:
            if ($type === 'BOTTOM_ENTRY') {
                // Only 1 loser in history at score 40.1 - stay above 40
                return $score >= 40.0;
            }
            ...
            if ($type === 'VWAP_RECLAIM_1M') {
                ...
                // ATR Filter: Relaxed to <2.5% for more volume
                // Original strict: <1.5%, now accepting medium volatility
                if ($atrPct > 2.5) {
                    return false;
                }

                // Volume Filter: Reject medium-high volume (3-5x range)
                // Data shows: 0-3x vol = 83.3% WR, 1.54% avg P&L
                //             3-5x vol = 0% WR (false breakouts)
                //             7x+ vol = 100% WR (explosive moves)
                if ($volRatio >= 3.0 && $volRatio < 7.0) {
                    return false;
                }

                // Score Filter: Need strong signal
                return $score >= 50.0 && ($score < 60.0 || $score >= 80.0);
            }

            if ($type === 'BULL_FLAG_BREAK') {
                // 22 losers with avg score 83.5 - stay in high range
                return $score >= 80.0;
            }

            if ($type === 'MIDDAY_CONTINUATION') {
                $atrPct = (float) ($c['atr_pct'] ?? 0);

                // Accept the discovered pattern: score 50-55, ATR 1.0-2.5%, 11am-1pm
                // Data: 11am hour = 100% WR, 5.91% P&L (5 trades)
                // Broader 11am-1pm = 50% WR, 1.38% P&L, 35.7% big winners
                if ($score < 50.0 || $score >= 55.0) {
                    return false;
                }
                if ($atrPct < 1.0 || $atrPct > 2.5) {
                    return false;
                }

                // Time already enforced in pattern detection (11am-1pm)
                return true;
            }

            // All other types already rejected above
            return false;
        });
        ...
        // Suggested trailing stop = config-driven ATR multiplier with bounds
