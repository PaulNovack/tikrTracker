...
/**
 * Version 55 - One-minute Trend Compression Continuation entry finder.
 *
 ...
 */
class OneMinuteEntryFinderV55
{
    use HasPriceTables;

    private string $version = 'v55';
    ...
    public float $minNotional1m = 50000.0;

    public float $minVolumeRatio = 1.10;

    public float $minBodyPct = 0.06;

    public float $minClosePosition = 0.65;

    public float $maxUpperWickFraction = 0.35;

    public float $maxAboveVwapPct = 1.80;

    public float $minAtrPct = 0.12;

    public float $maxAtrPct = 2.50;

    public float $minEmaSpreadPct = 0.00;

    public float $minEma9SlopePct = 0.005;

    public int $minFlagBars = 3;

    public int $maxFlagBars = 8;

    public float $maxFlagRangePct = 1.25;

    public float $maxFlagRangeAtr = 1.35;

    public float $breakBufferPct = 0.02;

    public float $maxBreakDistanceAtr = 0.45;

    public float $maxPullbackBelowVwapPct = 0.30;
    ...
    public float $stopAtrBuffer = 0.10;

    public float $stopPriceBufferPct = 0.03;

    public float $minStopPct = 0.30;

    public float $maxStopPct = 1.25;

    public float $minRoomToSessionHighR = 0.75;

    public float $sessionHighBreakTolerancePct = 0.05;

    public float $trailingStopAtrMultiplier = 2.0;
    ...
    public int $morningScoreSplitMinute = 630; // 10:30
    ...
    private function findEntry(
        ...
        for ($i = $lastIndex; $i >= $earliestIndex; $i--) {
            ...
            $suggestedTrail = $atr * $this->trailingStopAtrMultiplier;
        ...
        if ($candidates === []) {
            return [null, 'no_valid_v55_entry'];
        }
        ...
        if ($this->isDebugEnabled()) {
            Log::info('[EntryFinderV55] accepted entry', [
                'symbol' => $symbol,
    ...
    private function detectTightFlagBreakout(
        ...
        for ($window = $minFlagBars; $window <= $maxFlagBars; $window++) {
            ...
            // Require either a genuine pullback bar or visibly tightening ranges.
            if ($redBars === 0 && $compressionRatio > 0.80) {
                continue;
            }
    ...
    private function maybeLogDebug(): void
    {
        ...
        if (self::$dbg['called'] > 0 && self::$dbg['called'] % $every === 0) {
            Log::info('[EntryFinderV55] debug counters', array_merge(self::$dbg, [
                'pid' => getmypid(),
                'debug_every' => $every,
            ]));
        }
