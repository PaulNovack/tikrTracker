...
/**
 * Version 25.0 - Entry Finder (Quality-first)
 *
 * Entry types (long):
 ...
 */
class OneMinuteEntryFinderV25_1
{
    use HasPriceTables;

    private string $version = 'v25.1';
    ...
    private function findEntry(string $symbol, string $assetType, string $signalTsEst, string $asOfTsEst): ?array
    {
        ...
        // --- Risk/quality gates ---
        $minBars = (int) ($cfg['min_bars'] ?? 60);
        $minNotional1m = (float) ($cfg['min_notional_1m'] ?? 80000.0);  // last 1m bar notional floor
        $maxAboveVwapPct = (float) ($cfg['max_above_vwap_entry_pct'] ?? 0.80); // don't chase >0.8% above VWAP
        $minRoomPct = (float) ($cfg['min_room_to_run_pct'] ?? 1.0);      // must have >=1% room to HOD or runway
        $roomAtrMult = (float) ($cfg['room_atr_mult'] ?? 2.0);           // OR (HOD-entry) >= ATR*mult
        $minVolRatio = (float) ($cfg['min_vol_ratio_1m'] ?? 2.0);        // vs avg last 20m
        ...
        $beforeMinutes = (int) ($cfg['analysis_lookback_minutes'] ?? 30);
        ...
        $bars = $this->dbSelect('
            ...
            WHERE asset_type = ?
              ...
              AND ts_est <= ?
            ORDER BY ts_est ASC
        ', [$assetType, $symbol, $marketOpen, $analysisEnd]);
        ...
        for ($i = max(1, $idxStart); $i < count($norm); $i++) {
            ...
            if ($c['close'] <= $c['open'] || $bodyPct < 0.25) {
                continue;
            }
            ...
            $score = ($vr * 1.2) + max(0.0, 1.5 - abs($aboveVwapPct)) + ($atrPct * 0.3);
            $score *= $this->timeMultiplier($c['ts']);
            ...
            $candidates[] = $this->packEntry(
                ...
                [
                    ...
                    'hod' => $c['hod'],
                    'or_high' => $c['or_high'],                    'five_min_directional_changes' => $choppiness['directional_changes'] ?? null,
                    'five_min_green_bar_pct' => isset($choppiness['green_bar_pct']) ? round($choppiness['green_bar_pct'], 1) : null,
                    'five_min_net_progress' => isset($choppiness['net_progress']) ? round($choppiness['net_progress'], 3) : null,                    'five_min_directional_changes' => $choppiness['directional_changes'] ?? null,
                    'five_min_green_bar_pct' => isset($choppiness['green_bar_pct']) ? round($choppiness['green_bar_pct'], 1) : null,
        ...
        for ($i = max(6, $idxStart); $i < count($norm); $i++) {
            ...
            $bodyPct = ($c['open'] > 0) ? abs($c['close'] - $c['open']) / $c['open'] * 100.0 : 0.0;
            if ($c['close'] <= $c['open'] || $bodyPct < 0.30) {
                continue;
            }

            $base = $volAvg($i);
            $vr = ($base > 0) ? ($c['volume'] / $base) : 0.0;
            if ($vr < max(1.8, $minVolRatio)) {
                continue;
            }
            ...
            $score = ($vr * 1.1) + ($bodyPct * 0.15) + ($atrPct * 0.4);
            $score *= $this->timeMultiplier($c['ts']);
        ...
        for ($i = max(10, $idxStart); $i < count($norm); $i++) {
            ...
            // green + body
            $bodyPct = ($c['open'] > 0) ? abs($c['close'] - $c['open']) / $c['open'] * 100.0 : 0.0;
            if ($c['close'] <= $c['open'] || $bodyPct < 0.20) {
                continue;
            }
            ...
            $score = ($vr * 1.0) + ($atrPct * 0.5) + max(0.0, 1.0 - abs($aboveVwapPct));
            $score *= $this->timeMultiplier($c['ts']);
        ...
        // v26.0: DATA-DRIVEN FILTER - Join analysis: Score 40-59 = 5.40% P/L!
        // Accept score 40-59 OR 80-100, REJECT 0-39 and 60-79
        $candidates = array_filter($candidates, function ($c) {
            ...
            // Only proven types with 56%+ WR and 1.0%+ P/L
            $allowedTypes = [
                'BOTTOM_ENTRY', 'EMA21_BOUNCE', 'MA_SQUEEZE', 'OR_BREAKOUT',
                'ORB_RETEST', 'BULL_FLAG_BREAK', 'VWAP_RECLAIM_1M',
                'EMA_CROSS_BULL', 'BREAKOUT_RETEST', 'EMA9_PULLBACK',
            ];
            if (! in_array($type, $allowedTypes)) {
                return false;
            }

            // Score 40-59 OR 80-100 only
        ...
        $minStop = (float) $best['entry'] * (1 - ($maxStopPct / 100)); // Lower price = wider stop
        $maxStop = (float) $best['entry'] * (1 - ($minStopPct / 100)); // Higher price = tighter stop
