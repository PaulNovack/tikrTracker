...
class YFinanceStocks5MinCommand extends Command
{
    use ManagesCacheAfterDataUpdate;

    protected $signature = 'market:yfinance-stocks-5min {hoursBack=24} {--limit= : Limit number of symbols to process} {--offset=0 : Skip first N symbols (for batching)} {--watched-only : Process only watched symbols (overrides config)}';

    protected $description = 'Fetch 5-minute interval stock prices from Yahoo Finance using Python yfinance';
    ...
    public function handle(): int
    {
        ...
        $scriptPath = $pythonDir.'/yfinance_stocks_5min.py';
        ...
        // Get stock symbols from database, prioritizing watched symbols first
        $watchedSymbols = $this->getWatchedSymbols('stock');
        ...
        if ($onlyWatched) {
            ...
            $allSymbols = $this->getAllSymbols('stock');
        ...
        $this->info('=== YFinance Stocks 5-Minute Interval Sync ===');
        $this->info("Fetching last {$hoursBack} hours of 5-minute stock data...");
        ...
        if ($onlyWatched) {
            ...
            if (! empty($watchedSymbols)) {
                ...
                $this->processSymbolBatch($watchedSymbols, $hoursBack, 'watched stocks');
            ...
            if (! empty($watchedSymbols)) {
                ...
                if (! empty($watchedToProcess)) {
                    ...
                    $this->processSymbolBatch($watchedToProcess, $hoursBack, 'watched stocks');
            ...
            if ($limit === null || $limit > 0) {
                ...
                if (! empty($unwatchedToProcess)) {
                    ...
                    $this->processSymbolBatch($unwatchedToProcess, $hoursBack, 'unwatched stocks');
    ...
    /**
     * Process a batch of symbols through the Python script
     */
    private function processSymbolBatch(array $symbols, int $hoursBack, string $description): void
    {
        if (empty($symbols)) {
            return;
        }
        ...
        $process->run(function ($type, $buffer) use ($description) {
            $this->handleProcessOutput($type, $buffer, "YFinance Stocks 5min - {$description}");
        });
