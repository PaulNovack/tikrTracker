...
class MarketYfinanceCrypto1minBatch extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'market:yfinance-crypto-1min-batch 
     ...
     * @var string
     */
    protected $description = 'Fetch 1-minute crypto data using optimized batch requests (multiple symbols per API call)';
    ...
    public function handle(): int
    {
        ...
        $this->info('🚀 Starting optimized batch 1-minute crypto sync');
        ...
        $pythonScript = base_path('python/yfinance_crypto_1min_batch.py');
        $command = [
            'python',
            $pythonScript,
            (string) $hours,
        ...
        if ($returnCode === 0) {
            $this->info("✅ Batch crypto sync completed successfully in {$duration}s");

            return self::SUCCESS;
        } else {
            $this->error("❌ Batch crypto sync failed with exit code {$returnCode}");
