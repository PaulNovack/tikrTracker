...
class BackfillOneMinutePricesFull extends Command
{
    private const MAX_PLACEHOLDERS_PER_INSERT = 65000;

    protected $signature = 'trading:backfill-one-minute-prices-full
    ...
    protected $description = 'Insert missing rows from one_minute_prices into one_minute_prices_full';
    ...
    public function handle(): int
    {
        ...
        if ($missingCount === 0) {
            $this->info('No missing one-minute rows found in one_minute_prices_full.');
        ...
        $this->info("Missing one-minute rows found: {$missingCount}");
        $this->line('Source: one_minute_prices');
        $this->line('Target: one_minute_prices_full');
        ...
        while ($processed < $insertCount) {
            ...
            DB::table('one_minute_prices_full')->insertOrIgnore($payload);
        ...
        $deleted = DB::table('one_minute_prices')->where('ts', '<', $cutoff)->delete();
        $this->info("Pruned {$deleted} rows older than 45 days from one_minute_prices.");
    ...
    {
        $query = DB::table('one_minute_prices as src')
            ->leftJoin('one_minute_prices_full as dest', function ($join): void {
