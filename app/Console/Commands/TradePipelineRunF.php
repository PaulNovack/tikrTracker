...
class TradePipelineRunF extends Command
{
    use TracesPipelineRun;

    protected $signature = 'trade:pipeline-f
    ...
    protected $description = 'Run Pipeline F (uses TRADE_ALERT_F_VERSION): 5m scan -> 1m entry refine -> store alerts to DB (live or backtest).';
    ...
    public function handle(TradeAlertWriterV1 $writer): int
    {
        ...
        if (! $this->option('backtest') && ! $this->option('rolling-window') && ! TradingSettingService::isPipelineRunCronEnabled('f')) {
            $this->info('Pipeline F: Execution disabled (trading.pipeline_f.run_cron=0). Exiting.');

            return 0;
        }

        $version = config('app.trade_alert_f_version', 'v70.0');
        ...
        if (! class_exists($scannerClass) || ! class_exists($finderClass)) {
            $this->error("Pipeline F version {$version} not found (Scanner: {$scannerClass}, Finder: {$finderClass})");
        ...
        // ── Redis gate ──
        if (config('trading.pipelines.f.use_redis', false)) {
            $msg = 'Pipeline F: Redis scanning active — SQL pipeline exiting (bar-events:consume handles alerts).';
            $this->info($msg);
        ...
        $tracer = $this->startTrace('F', $asOfTsEst);
        ...
        if (! $signals) {
            $this->line("Pipeline F ({$version}): No 5m signals at {$asOfTsEst}");
        ...
        $staleMinutes = TradingSettingService::getPipelineMaxAgeMinutes('f');
        ...
        // Get ignore types and no_filter_finder for Pipeline F from config
        $ignoreTypes = config('trading.pipelines.f.ignore_types', []);
        $noFilterFinder = config('trading.pipelines.f.no_filter_finder', false);
        ...
        foreach ($signals as $sig) {
            ...
            // Write alert with pipeline_run = 'F'
            $alertId = $writer->upsertAlert($sig, $entry, $asOfTsEst, $scanner->getVersion(), 'F');
            if ($alertId) {
                $alertsWritten++;
            ...
            $ranked[] = [
                ...
                'entryTs' => $entry['entry_ts_est'] ?? null,
        ...
        $this->info("Pipeline F ({$version}) | As-of (EST): {$asOfTsEst}");
            ...
            for ($t = $tStart; $t <= $tEnd; $t += ($step * 60)) {
                ...
                foreach ($signals as $sig) {
                    ...
                    $res = $finder->findBestLong(
                        ...
                        (string) $this->option('fill'),
                        (int) $this->option('stale'),
                        $sig['meta'] ?? [] // Pass scanner metadata to entry finder
                    );
                    ...
                    // Write alert with pipeline_run = 'F'
                    if ($writer->upsertAlert($sig, $entry, $asOfTsEst, $scanner->getVersion(), 'F')) {
                        $totalAlerts++;
                    }
                }
            }

            $this->line("Pipeline F ({$version}): Backtest day {$day} done.");
        }

        $this->info("Pipeline F ({$version}): Backtest complete.");
    ...
    private function runRollingWindowBacktest($scanner, $finder, TradeAlertWriterV1 $writer, string $assetType, string $version): int
    {
        ...
        $this->info("Pipeline F ({$version}): Rolling window backtest");
