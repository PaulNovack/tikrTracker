...
class TradePipelineRunD extends Command
{
    use TracesPipelineRun;

    protected $signature = 'trade:pipeline-d
    ...
    protected $description = 'Run Pipeline D (uses TRADE_ALERT_D_VERSION): 5m scan -> 1m entry refine -> store alerts to DB (live or backtest).';
    ...
    public function handle(TradeAlertWriterV1 $writer): int
    {
        ...
        if (! $this->option('backtest') && ! $this->option('rolling-window') && ! TradingSettingService::isPipelineRunCronEnabled('d')) {
            $this->info('Pipeline D: Execution disabled (trading.pipeline_d.run_cron=0). Exiting.');

            return 0;
        }

        $version = config('app.trade_alert_d_version', 'v26.0');

        // Convert version format: v26.0 -> V26_0
        ...
        if (! class_exists($scannerClass) || ! class_exists($finderClass)) {
            $this->error("Pipeline D version {$version} not found (Scanner: {$scannerClass}, Finder: {$finderClass})");
        ...
        $tracer = $this->startTrace('D', $asOfTsEst);
        ...
        if (! $signals) {
            $this->line("Pipeline D ({$version}): No 5m signals at {$asOfTsEst}");
        ...
        $staleMinutes = TradingSettingService::getPipelineMaxAgeMinutes('d');

        // Filter out stale 5-minute signals immediately (before passing to finder)
        if ($staleMinutes > 0) {
            ...
            foreach ($signals as $sig) {
                ...
                $signalAgeMinutes = round(($nowEpoch - $signalEpoch) / 60, 1);

                if (($nowEpoch - $signalEpoch) > ($staleMinutes * 60)) {
                    $this->info("STALE SIGNAL: Skipping {$sig['symbol']} - 5m signal is {$signalAgeMinutes} min old (limit: {$staleMinutes} min). Now: {$actualNowEst}, Signal: {$sig['signal_ts_est']}");
            ...
            if (empty($signals)) {
                $this->line("Pipeline D ({$version}): All 5m signals were stale (>{$staleMinutes} min old)");
                $tracer?->finish(['alerts_written' => 0, 'signals' => 0]);
        ...
        // Use actual current time for entry stale checks (calculated once for consistency)
        $actualNowEst = now('America/New_York')->format('Y-m-d H:i:s');
        $nowEpoch = strtotime($actualNowEst);
        ...
        // Get ignore types and no_filter_finder for Pipeline D from config
        $ignoreTypes = config('trading.pipelines.d.ignore_types', []);
        $noFilterFinder = config('trading.pipelines.d.no_filter_finder', false);
        ...
        foreach ($signals as $sig) {
            ...
            // Write alert with pipeline_run = 'D'
            $alertId = $writer->upsertAlert($sig, $entry, $asOfTsEst, $scanner->getVersion(), 'D', true);
            if ($alertId) {
                $alertsWritten++;
        ...
        $this->info("Pipeline D ({$version}) | As-of (EST): {$asOfTsEst}");
        ...
        for ($d = $startDate; $d <= $endDate; $d += 86400) {
            ...
            for ($t = $tStart; $t <= $tEnd; $t += ($step * 60)) {
                ...
                foreach ($signals as $sig) {
                    ...
                    if (empty($res['ok']) || empty($res['best_entry'])) {
                        $reason = $res['filter_reason'] ?? $res['error'] ?? 'Unknown';
                        if ($reason === 'Unknown' && isset($res['ok']) && $res['ok'] === false) {
                            $reason = 'No pattern matched or all candidates filtered';
                        }
                        $this->line("[{$sig['symbol']}] Rejected: {$reason}");
                    }
                    ...
                    // Write alert with pipeline_run = 'D'
                    if ($writer->upsertAlert($sig, $entry, $asOfTsEst, $scanner->getVersion(), 'D', false)) {
                        $totalAlerts++;
                    }
                }
            }

            $this->line("Pipeline D ({$version}): Backtest day {$day} done.");
        }

        $this->info("Pipeline D ({$version}): Backtest complete.");
    ...
    private function runRollingWindowBacktest($scanner, $finder, TradeAlertWriterV1 $writer, string $assetType, string $version): int
    {
        ...
        $fromDate = $est->copy()->subMinutes(12)->format('Y-m-d');
        $toDate = $est->copy()->addMinutes(6)->format('Y-m-d');
        $timeFrom = $est->copy()->subMinutes(12)->format('H:i:00');
        ...
        $this->info("Pipeline D ({$version}): Rolling window backtest");
