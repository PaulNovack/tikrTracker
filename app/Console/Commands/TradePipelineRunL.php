...
class TradePipelineRunL extends Command
{
    ...
    protected $signature = 'trade:pipeline-l
        ...
        {--stale=10 : live mode: ignore entries older than N minutes}
    ...
    protected $description = 'Run Pipeline L (uses TRADE_ALERT_L_VERSION): 5m scan -> 1m entry refine -> store alerts to DB (live or backtest).';
    ...
    public function handle(TradeAlertWriterV1 $writer): int
    {
        ...
        if (! $this->option('backtest') && ! $this->option('rolling-window') && ! TradingSettingService::isPipelineRunCronEnabled('l')) {
            $this->info('Pipeline L: Execution disabled (trading.pipeline_l.run_cron=0). Exiting.');

            return 0;
        }
        $version = config('app.trade_alert_l_version', 'v17.0');
        ...
        if (! class_exists($scannerClass) || ! class_exists($finderClass)) {
            $this->error("Pipeline L version {$version} not found (Scanner: {$scannerClass}, Finder: {$finderClass})");
        ...
        // ── Redis gate ──
        if (config('trading.pipelines.l.use_redis', false)) {
            $msg = 'Pipeline L: Redis scanning active — SQL pipeline exiting (bar-events:consume handles alerts).';
            $this->info($msg);
        ...
        if ($this->option('backtest')) {
            return $this->runBacktest(
                ...
                $assetType,
                $version,
                (bool) $this->option('enforce-current-freshness')
            );
        }

        $asOfTsEst = $this->resolveAsOfTsEst((string) $this->option('asOf'));
        $tracer = $this->startTrace('L', $asOfTsEst);
        ...
        if (! $signals) {
            $this->line("Pipeline L ({$version}): No 5m signals at {$asOfTsEst}");
        ...
        // Fallback to Pipeline L max-age config only when option is omitted/invalid.
        $staleMinutes = (int) $this->option('stale');
        if ($staleMinutes <= 0) {
            $staleMinutes = TradingSettingService::getPipelineMaxAgeMinutes('l');
        }
        ...
        // Get ignore types and no_filter_finder for Pipeline L from config
        $ignoreTypes = config('trading.pipelines.l.ignore_types', []);
        $noFilterFinder = config('trading.pipelines.l.no_filter_finder', false);
        ...
        foreach ($signals as $sig) {
            ...
            if ($staleMinutes > 0 && ($nowEpoch - $signalEpoch) > ($staleMinutes * 60)) {
                ...
                \Log::warning('[Pipeline L] Early stale signal detected in live path', [
        ...
        foreach ($candidates as $candidate) {
            $alertId = $writer->upsertAlert($candidate['sig'], $candidate['entryPayload'], $asOfTsEst, $scanner->getVersion(), 'L', true);
            if ($alertId) {
                $alertsWritten++;
                $tracer?->alertWritten($alertId, $candidate['sig']['symbol'], $candidate['entryPayload']['entry_ts_est'], $candidate['sig']['signal_ts_est']);
            }
        }

        $tracer?->finish(['alerts_written' => $alertsWritten, 'signals' => count($signals)]);

        $this->info("Pipeline L ({$version}) | As-of (EST): {$asOfTsEst}");
    ...
    private function runBacktest($scanner, $finder, TradeAlertWriterV1 $writer, string $assetType, string $version, bool $enforceCurrentFreshness = false): int
    {
        ...
        $slotsPerDay = (int) floor((strtotime('1970-01-01 '.$timeTo) - strtotime('1970-01-01 '.$timeFrom)) / ($step * 60)) + 1;
        if ($slotsPerDay < 1) {
            $this->error('Invalid time window: --timeTo must be greater than or equal to --timeFrom');

            return 1;
        }

        $totalDays = (int) floor(($endDate - $startDate) / 86400) + 1;
        if ($totalDays < 1) {
            $this->error('Invalid date range: --to must be greater than or equal to --from');

            return 1;
        }

        $totalSlots = $slotsPerDay * $totalDays;
        ...
        $this->info("Pipeline L ({$version}): Backtest {$from} -> {$to} | {$totalDays} day(s), {$slotsPerDay} slot(s)/day, {$totalSlots} total slot(s)");
        $progressBar = $this->output->createProgressBar($totalSlots);
        ...
        for ($d = $startDate; $d <= $endDate; $d += 86400) {
            ...
            for ($t = $tStart; $t <= $tEnd; $t += ($step * 60)) {
                ...
                foreach ($signals as $sig) {
                    ...
                    if ($enforceCurrentFreshness && $staleMinutes > 0) {
                        ...
                        if ($entryAgeMinutes > $staleMinutes) {
                            ...
                            \Log::channel('stale-alerts')->warning('[Pipeline L] Early stale alert candidate detected before write', [
                                'symbol' => $sig['symbol'] ?? null,
                                'signal_ts_est' => $sig['signal_ts_est'] ?? null,
                ...
                foreach ($slotCandidates as $candidate) {
                    if ($writer->upsertAlert($candidate['sig'], $candidate['entry'], $asOfTsEst, $scanner->getVersion(), 'L', false)) {
                        $totalAlerts++;
                    }
                }
            ...
            $this->line("Pipeline L ({$version}): Backtest day {$day} done.");
        }

        $progressBar->finish();
        $this->newLine(2);

        $this->info("Pipeline L ({$version}): Backtest complete.");
    ...
    private function runRollingWindowBacktest($scanner, $finder, TradeAlertWriterV1 $writer, string $assetType, string $version): int
    {
        ...
        $this->info("Pipeline L ({$version}): Rolling window backtest");
