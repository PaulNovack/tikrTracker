...
class TradePipelineRunA extends Command
{
    use TracesPipelineRun;

    protected $signature = 'trade:pipeline-a
    ...
    protected $description = 'Run Pipeline A (uses TRADE_ALERT_A_VERSION): 5m scan -> 1m entry refine -> store alerts to DB (live or backtest).';
    ...
    public function handle(TradeAlertWriterV1 $writer): int
    {
        ...
        if (! $this->option('backtest') && ! $this->option('rolling-window') && ! TradingSettingService::isPipelineRunCronEnabled('a')) {
            $this->info('Pipeline A: Execution disabled (trading.pipeline_a.run_cron=0). Exiting.');

            return 0;
        }

        $version = config('app.trade_alert_a_version', 'v17.0');
        ...
        if (! class_exists($scannerClass) || ! class_exists($finderClass)) {
            $this->error("Pipeline A version {$version} not found (Scanner: {$scannerClass}, Finder: {$finderClass})");
        ...
        // ── Redis gate ──
        if (config('trading.pipelines.a.use_redis', false)) {
            $msg = 'Pipeline A: Redis scanning active — SQL pipeline exiting (bar-events:consume handles alerts).';
            $this->info($msg);
        ...
        $tracer = $this->startTrace('A', $asOfTsEst);
        ...
        if (! $signals) {
            $this->line("Pipeline A ({$version}): No 5m signals at {$asOfTsEst}");
        ...
        $staleMinutes = TradingSettingService::getPipelineMaxAgeMinutes('a');
        ...
        // Get ignore types and no_filter_finder for Pipeline A from config
        $ignoreTypes = config('trading.pipelines.a.ignore_types', []);
        $noFilterFinder = config('trading.pipelines.a.no_filter_finder', false);
        ...
        foreach ($signals as $sig) {
            ...
            if (empty($res['ok']) || empty($res['best_entry'])) {
                if ($noFilterFinder) {
                    ...
                    if ($totalEntries < 3) {
                        $this->info("DEBUG FILTERED_OUT: symbol={$sig['symbol']}, atr={$atr}, currentPrice={$currentPrice}, suggestedStop={$suggestedTrailingStop}");
                    }
            ...
            // Live freshness: reject if SIGNAL is too old (entry might be recent but signal could be stale)
            if ($staleMinutes > 0 && ($nowEpoch - $signalEpoch) > ($staleMinutes * 60)) {
                continue;
            }

            // Write alert with pipeline_run = 'A'
            $alertId = $writer->upsertAlert($sig, $entry, $asOfTsEst, $scanner->getVersion(), 'A');
            if ($alertId) {
                $alertsWritten++;
        ...
        $tracer?->checkpoint('FINDER_DONE', ['alerts_written' => $alertsWritten]);
        ...
        $this->info("Pipeline A ({$version}) | As-of (EST): {$asOfTsEst}");
        ...
        for ($d = $startDate; $d <= $endDate; $d += 86400) {
            ...
            for ($t = $tStart; $t <= $tEnd; $t += ($step * 60)) {
                ...
                foreach ($signals as $sig) {
                    ...
                    // Write alert with pipeline_run = 'A'
                    if ($writer->upsertAlert($sig, $entry, $asOfTsEst, $scanner->getVersion(), 'A')) {
                        $totalAlerts++;
                    }
                }
            }

            $this->line("Pipeline A ({$version}): Backtest day {$day} done.");
        }

        $this->info("Pipeline A ({$version}): Backtest complete.");
    ...
    private function runRollingWindowBacktest($scanner, $finder, TradeAlertWriterV1 $writer, string $assetType, string $version): int
    {
        ...
        $this->info("Pipeline A ({$version}): Rolling window backtest");
