...
class RedisBarHydrator extends Command
{
    ...
    public function handle(): int
    {
        ...
        // Use the latest bar timestamp in the database as the baseline start,
        // but extend the end to "now" (clamped to market hours) so premarket
        // bars are hydrated even when the latest DB bar is from yesterday.
        $dbLatest = $this->latestBarTimestamp('one_minute_prices', $assetType, $symbols);
        ...
        // Clamp DB anchor to 4:00 PM EST (20:00 UTC) — bars past market close
        // are after-hours and not useful for session-wide indicator computation.
        $sessionCloseUtc = $dbLatest->copy()->setTime(20, 0, 0);
        if ($dbLatest->gt($sessionCloseUtc)) {
            $dbLatest = $sessionCloseUtc;
        }

        // Compute "now" in EST, clamped: weekends → Friday 4pm, after-hours → 4pm
        $nowEst = Carbon::now('America/New_York');
        if ($nowEst->isWeekend()) {
            $nowEst = $nowEst->copy()->previous(Carbon::FRIDAY)->setTime(16, 0, 0);
        } elseif ($nowEst->gt($nowEst->copy()->setTime(16, 0, 0))) {
            $nowEst = $nowEst->copy()->setTime(16, 0, 0);
        }
        $nowUtc = $nowEst->copy()->setTimezone('UTC');

        // Start from the DB baseline, end extends to now for premarket coverage
        $start1m = $dbLatest->copy()->subMinutes($minutes1m);
        $latest1m = $dbLatest->max($nowUtc);

        $this->info(sprintf(
            '  DB anchor: %s UTC | End: %s UTC',
            $dbLatest->toDateTimeString(),
    ...
    private function write1mBarsToRedis(string $symbol, string $assetType, array $bars, bool $skipEvents): int
    {
        ...
        foreach ($bars as $bar) {
            ...
            $dateStr = Carbon::parse($bar->ts)->setTimezone('America/New_York')->format('Ymd');
            ...
            $pipe = RedisFacade::connection(self::RT_CONNECTION)->pipeline();
    ...
    private function write5mBarsToRedis(string $symbol, string $assetType, array $bars, bool $skipEvents): int
    {
        ...
        foreach ($bars as $bar) {
            ...
            $dateStr = Carbon::createFromTimestamp($epoch, 'America/New_York')->format('Ymd');
    ...
    {
        $dateStr = Carbon::now('America/New_York')->format('Ymd');
        $key = sprintf('%shydrated:%s:%s', self::RT_PREFIX, $dateStr, $assetType);
        RedisFacade::connection(self::RT_CONNECTION)->set($key, '1', 'EX', self::HYDRATION_TTL_SECONDS);
