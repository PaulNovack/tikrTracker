...
class TradePipelineRunI extends Command
{
    use TracesPipelineRun;

    protected $signature = 'trade:pipeline-i
    ...
    protected $description = 'Run Pipeline I (uses TRADE_ALERT_I_VERSION): 5m scan -> 1m entry refine -> store alerts to DB (live or backtest).';
    ...
    public function handle(TradeAlertWriterV1 $writer): int
    {
        ...
        if (! $this->option('backtest') && ! $this->option('rolling-window') && ! TradingSettingService::isPipelineRunCronEnabled('i')) {
            $this->info('Pipeline I: Execution disabled (trading.pipeline_i.run_cron=0). Exiting.');

            return 0;
        }
        $version = config('app.trade_alert_i_version', 'v17.0');
        ...
        if (! class_exists($scannerClass) || ! class_exists($finderClass)) {
            $this->error("Pipeline I version {$version} not found (Scanner: {$scannerClass}, Finder: {$finderClass})");
        ...
        // ── Redis gate ──
        if (config('trading.pipelines.i.use_redis', false)) {
            $msg = 'Pipeline I: Redis scanning active — SQL pipeline exiting (bar-events:consume handles alerts).';
            $this->info($msg);
        ...
        $tracer = $this->startTrace('I', $asOfTsEst);
        ...
        if (! $signals) {
            $this->line("Pipeline I ({$version}): No 5m signals at {$asOfTsEst}");
        ...
        // Fallback to Pipeline I max-age config only when option is omitted/invalid.
        $staleMinutes = (int) $this->option('stale');
        if ($staleMinutes <= 0) {
            $staleMinutes = TradingSettingService::getPipelineMaxAgeMinutes('i');
        }
        ...
        // Get ignore types and no_filter_finder for Pipeline I from config
        $ignoreTypes = config('trading.pipelines.i.ignore_types', []);
        $noFilterFinder = config('trading.pipelines.i.no_filter_finder', false);
        ...
        foreach ($signals as $sig) {
            ...
            if ($staleMinutes > 0 && ($nowEpoch - $signalEpoch) > ($staleMinutes * 60)) {
                ...
                \Log::warning('[Pipeline I] Early stale signal detected in live path', [
            ...
            // Write alert with pipeline_run = 'I'
            $alertId = $writer->upsertAlert($sig, $entry, $asOfTsEst, $scanner->getVersion(), 'I', true);
            if ($alertId) {
                $alertsWritten++;
        ...
        $this->info("Pipeline I ({$version}) | As-of (EST): {$asOfTsEst}");
                ...
                foreach ($signals as $sig) {
                    ...
                    if ($enforceCurrentFreshness && $staleMinutes > 0) {
                        ...
                        if ($entryAgeMinutes > $staleMinutes) {
                            $totalStaleDetected++;
                            \Log::channel('stale-alerts')->warning('[Pipeline I] Early stale alert candidate detected before write', [
                                'symbol' => $sig['symbol'] ?? null,
                                'signal_ts_est' => $sig['signal_ts_est'] ?? null,
                    ...
                    if ($writer->upsertAlert($sig, $entry, $asOfTsEst, $scanner->getVersion(), 'I', false)) {
                        $totalAlerts++;
                    }
                }
            }

            $this->line("Pipeline I ({$version}): Backtest day {$day} done.");
        }

        $this->info("Pipeline I ({$version}): Backtest complete.");
    ...
    private function runRollingWindowBacktest($scanner, $finder, TradeAlertWriterV1 $writer, string $assetType, string $version): int
    {
        ...
        $this->info("Pipeline I ({$version}): Rolling window backtest");
