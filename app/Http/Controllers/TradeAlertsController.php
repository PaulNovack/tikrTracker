    ...
    private function fetchAllAlerts(string $tableName = 'trade_alerts', int|float $mlMinThreshold = 0, string $dateFilter = '', string $symbolFilter = '', string $pipelineFilter = '')
    {
        $query = DB::table($tableName)
            ...
            ->select([
                ...
                $tableName.'.risk_pct',
                $tableName.'.score',
                $tableName.'.sentiment_boost',
                $tableName.'.sentiment_score_1_100',
                $tableName.'.vol_ratio',
                $tableName.'.atr',
                $tableName.'.atr_pct',
        ...
        $alerts->getCollection()->transform(function ($alert) {
            ...
            return [
                ...
                'sentimentBoost' => isset($alert->sentiment_boost) ? (float) $alert->sentiment_boost : null,
                'sentimentScore' => isset($alert->sentiment_score_1_100) ? (int) $alert->sentiment_score_1_100 : null,
